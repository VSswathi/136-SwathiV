...
function MyCalculatorWithHook(props){
  ...
  return(<>
    <div>
      <p>result of {first} and {second} is {result}</p>
      <button onClick={handleResult}>Addition</button>
    </div>
    <div>
    <button onClick={handleResult1}>subtraction</button>
  </div>
    <div>
    <button onClick={handleResult2}>multiplication</button>
  </div>
  <div>
    <button onClick={handleResult3}>division</button>
  </div>
  </>);
